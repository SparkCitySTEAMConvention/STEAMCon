...
test('backend dispatch uses exact scopes, authenticated author and READ/POST contracts', async () => {
  ...
  const source = createSpeakerForumSource(repository, user, 'backend')
  ...
  assert.deepEqual(calls, [
    ...
    ['messages', 'forum-id', 'SPEAKER', 'READ'],
    ['post', 'forum-id', { authorId: user.id, body: 'Hello', role: 'SPEAKER', permission: 'POST' }],
  ])
})

test('backend failures propagate for page retry and draft preservation', async () => {
  const fail = async () => { throw new Error('Unavailable') }
  const source = createSpeakerForumSource({ getForums: fail, getMessages: fail, createMessage: fail }, user, 'backend')
