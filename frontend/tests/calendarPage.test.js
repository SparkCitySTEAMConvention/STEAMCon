...
test('/calendar is public, reachable ahead of the protected routes, and links exist in navigation', async () => {
  ...
  try {
    ...
    const source = await readFileAsync(new URL('../src/App.jsx', import.meta.url), 'utf8')
...
test('preview calendar groups approved sessions by convention day and filters toggle by STEAM category', async t => {
  const dom = new JSDOM('<div id="root"></div>', { url: 'https://steamcon.test/calendar' })
  ...
  try {
    ...
    assert.match(resultCount(), /Showing 6 events across 3 days/)
    assert.deepEqual([...document.querySelectorAll('.calendar-day h3')].map(node => node.textContent.slice(0, 5)), ['Tuesd', 'Wedne', 'Thurs'])
