...
export function createAttendeeForumSource(repository, user, authSource) {
  const demo = authSource === 'demo' && user?.role === 'ATTENDEE'
  const available = demo || (authSource === 'backend' && user?.role === 'ATTENDEE' && uuid.test(user?.id || ''))
  const messages = []
  let nextId = 0
  function requireIdentity() {
    if (!available) throw new Error('A verified attendee account is required.')
  }
  return {
    demo, available,
    ...
    async getMessages(forumId) {
      requireIdentity()
      if (!demo) return repository.getMessages(forumId, 'ATTENDEE', 'READ')
    ...
    async createMessage(forumId, { body }) {
      ...
      const payload = { authorId: user.id, body: body.trim(), role: 'ATTENDEE', permission: 'POST' }
