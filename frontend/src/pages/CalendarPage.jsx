  ...
  return <>
    ...
    <main id="main" tabIndex={-1} className="calendar-page">
      ...
      <section className="container calendar-section">
        <p role="status">{live ? 'Live program · Published session occurrences are displayed in Eastern Time.' : source.mode === 'preview' ? 'Program preview · Times, rooms, and speakers are demonstration data.' : 'Live program unavailable · Sign in with a valid backend session.'}</p>
        ...
        {resource.status === 'ready' && <CalendarView sessions={resource.data.schedule || []} live={live} />}
      </section>
