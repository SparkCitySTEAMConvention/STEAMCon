...
import { createAttendeeForumSource } from '../services/attendeeForumSource.js'
import logo from '../assets/steamcon-logo.png'
import '../pages/speaker/SpeakerDashboard.css'
import '../pages/speaker/SpeakerForums.css'

const scopes = { TRACK: 'Track', ADMIN: 'Admin', CONCIERGE: 'Concierge' }

export default function ForumPage({ repository = forumRepository, audience = 'SPEAKER' }) {
  const attendee = audience === 'ATTENDEE'
  const label = attendee ? 'Attendee' : 'Speaker'
  const portalPath = attendee ? '/attendee' : '/speaker'
  const createSource = attendee ? createAttendeeForumSource : createSpeakerForumSource
  const { user, authSource } = useAuth()
  const source = useMemo(() => createSource(repository, user, authSource), [createSource, repository, user, authSource])
  ...
  return <div className="speaker-portal speaker-forums">
    ...
    <header className="portal-header">
      <div className="container portal-header-inner">
        <div className="portal-brand"><Link to="/" aria-label="STEAM Con home"><img src={logo} width="1828" height="860" alt="STEAM Con" /></Link><span>{label} Portal</span></div>
        <Link className="portal-home" to={portalPath}>Back to {label.toLowerCase()} dashboard</Link>
      </div>
      <div className="container"><AccountNavigation /></div>
    </header>
    <main id="forums-main" className="container portal-main" tabIndex={-1}>
      <div className="portal-welcome"><p className="eyebrow">Keep the conversation going</p><h1>{label} Forum &amp; Messaging</h1><p>Exchange ideas in track forums, connect with organizers, or ask the concierge for help.</p></div>
      ...
      {!source.available && <p role="alert">Forum access requires a signed-in {label.toLowerCase()} account. Please log in again if your session has expired.</p>}
    ...
    <footer className="container portal-footer"><p>STEAM Con · A place for curious minds.</p><p>{label} Forum &amp; Messaging</p></footer>
  </div>
}
  ...
  return <section className="forum-conversation" aria-labelledby="conversation-heading">
    ...
    {status === 'ready' && <>
      ...
      <form className="portal-editor" onSubmit={post} aria-busy={sending}>
        ...
        <p id="forum-post-help" className="portal-muted">{repository.demo ? 'This message will be added to the local demo conversation.' : canPost ? 'Posting requires POST permission for this forum.' : 'Posting requires a verified account with forum access.'}</p>
