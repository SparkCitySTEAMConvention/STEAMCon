...
export default function PortalSidebar({ user, onNavigate, collapsed = false, id }) {
      ...
      {groups.map(group => (
        <section key={group} aria-label={group}>
          ...
          <ul>{navigation.filter(item => item.group === group).map(item => (
            <li key={item.id}>
              {item.to ? (
                <Link title={collapsed ? item.label : undefined} to={item.to} onClick={onNavigate} aria-current={isPortalDestinationActive(item.to, location, item.activePaths) ? 'page' : undefined}>
