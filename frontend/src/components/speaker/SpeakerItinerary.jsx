...
export default function SpeakerItinerary({ source }) {
  ...
  return <section className="portal-itinerary portal-detail-section" id="speaker-itinerary" tabIndex={-1} aria-labelledby="speaker-itinerary-heading">
    ...
    <p className="portal-muted">Enrolled sessions are attendance plans. Speaking assignments appear separately below. Titles and locations are to be announced.</p>
    ...
    {resource.status === 'ready' && (resource.data.length ? <ol className="portal-itinerary-list">
    ...
    </ol> : <p>{source.demo ? 'No personal itinerary entries in this preview.' : 'No personal itinerary entries yet. Bookings and enrolled sessions will appear here when available.'}</p>)}
  </section>
}
