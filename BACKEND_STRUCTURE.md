...
| `communication.CommunicationService` | List/filter forums; find forum; derive caller identity and roles from the session and enforce forum policies; filter ACTIVE messages; require nonblank body and attach first matching speaker flair. CONCIERGE permits authenticated callers; other scopes require a matching policy. ID comparisons use Objects.equals. | [CommunicationService](backend/src/main/java/com/sparkcity/steamcon/communication/CommunicationService.java) |
...
| GET | `/api/forums/{id}/messages` | UUID path id; identity and roles from `X-Session-Id` | Message[] / 200 | CommunicationController |
| POST | `/api/forums/{id}/messages` | UUID path id, CreateMessageRequest | Message / 201 | CommunicationController |
| GET | `/api/notifications/me` | Authenticated user from `X-Session-Id`; no query/body identity | Notification[] / 200 | [NotificationController](backend/src/main/java/com/sparkcity/steamcon/communication/NotificationController.java) |
...
| `CreateMessageRequest` | `String body` (trimmed by frontend; author and roles derived from session) | [CreateMessageRequest](backend/src/main/java/com/sparkcity/steamcon/communication/CreateMessageRequest.java) |
...
Forum and notification controllers derive identity and roles from `X-Session-Id`. Notification list and mark-read enforce session ownership; forum messages accept only `{ body }`, and forum list supports optional scope. Other historical controller details in this document require separate verification. Proposal decisions do not enforce an ADMIN authority. Frontend role restrictions cannot establish backend authorization.
...
| [notificationRepository](frontend/src/services/notificationRepository.js) | GET `/api/notifications/me`; POST `/api/notifications/{id}/read` | Uses authenticatedFetch; requires a valid backend session and returns backend JSON unchanged; mark-read sends no body |
| [forumRepository](frontend/src/services/forumRepository.js) | GET forums with optional scope; GET messages without role/permission query; POST messages with only `{ body }` | Uses authenticatedFetch; requires a valid backend session; sends only trimmed body and returns backend JSON unchanged |
